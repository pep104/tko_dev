...
interface RouteApi {
    ...
    @GET("fleet/routes/")
    suspend fun getRoutesList(@Query("page") page: Int, @Query("page_size") pageSize: Int): ApiResult<RouteListResponse>

    @GET("fleet/routes/")
    suspend fun searchRoutesList(@Query("search") search: String): ApiResult<RouteListResponse>

    @GET("fleet/routes/{id}")
    suspend fun getRoute(@Path("id") id: Long): ApiResult<RouteData>
